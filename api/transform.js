export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'gender') {
      ...
      input = { prompt, negative_prompt: 'ugly, blurry, deformed', num_inference_steps: 25, guidance_scale: 7.5, width: 1024, height: 1024 };
    } else {
      version = '7de2ea26c616d5bf2245ad0d5e24f0ff9a6204578a5c876db53142edd9d2cd56';
      input = { image: dataUrl, codeformer_fidelity: 0.7, background_enhance: true, face_upsample: true, upscale: 2 };
    }
 
    // CORRIGIDO: Replicate usa "Token" não "Bearer"
    ...
    console.log('Prediction response:', JSON.stringify(prediction).slice(0, 300));
    ...
    while (Date.now() - start < 50000) {
      ...
      if (result.status === 'failed') {
        return res.status(500).json({ error: 'IA falhou: ' + (result.error || 'erro') });
      }
    }
